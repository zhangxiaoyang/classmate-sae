...
def parseText(msg):
    ...
    if querystr in ['帮助', 'help', '?', '？']:
        ...
        res += '- 插入 地理位置 修改当前位置\n'
        res += '- 发送「11位手机号」修改联系方式\n'
        res += '- 发送「hide/show」隐身/上线\n'
        res += '- 发送「status」查看当前状态\n'
        res += '- 发送「帮助/help/?」查看帮助\n'
        return res
        ...
        for c in classs:
            ...
            for i in Student.objects.filter(classs = c):
                if 'hide' in i.name:continue
                res += i.name + '\n'
        return res
    elif querystr in ['hide', 'show', 'status']:
        me = Student.objects.filter(weixin=openid)[0]
        lastname = me.name
        if querystr == 'hide':
            Student.objects.filter(weixin=openid).update(
                name=lastname.rstrip('hide')+querystr)
            return '您已成功隐身, 小伙伴们无法查看您的任何信息'
        elif querystr == 'show':
            Student.objects.filter(weixin=openid).update(
                name=lastname.rstrip('hide'))
            return '您已成功上线, 信息将被分享至同班同学'
        else:
            res = '隐身' if 'hide' in lastname else '上线'
            res = '您的状态为: %s\n' % res
            res += '----------------\n'
            res += '[姓名]' + i.name + '\n'
            res += '[学号]' + i.studentnum + '\n'
            if me.birthday != '':
                birth = ''
                if me.birthday[0] == 'g':birth = me.birthday[1:].encode('utf8')+'(公历)'
                else:birth = me.birthday[1:].decode('utf8')+'(农历)'
                res += '[破壳日]' + birth + '\n'
            
            addr = me.position.split('(')[0]
            res += '[地理位置]' + addr.encode('utf8') + '\n'
            if me.phone != '': res += '[联系方式]' + me.phone + '\n'
            if me.qq != '': res += '[QQ]' + me.qq + '\n'
            if me.weibo != '': res += '[微博]' + me.weibo + '\n'
            if me.mail != '': res += '[E-mail]' + me.mail + '\n'
            res += '\n'
            return res

    elif len(querystr) == 11:
        try:
            int(querystr)
            Student.objects.filter(weixin=openid).update(
            phone=querystr)
            res = '当前联系方式成功修改为: ' + querystr
            return res
        except:pass
    ...
    for c in classs:
        ...
        for i in Student.objects.filter(classs = c, name = querystr):
            ...
            if i.birthday != '':
                birth = ''
                if i.birthday[0] == 'g':birth = i.birthday[1:].encode('utf8')+'(公历)'
                else:birth = i.birthday[1:].decode('utf8')+'(农历)'
                res += '[破壳日]' + birth + '\n'
            ...
            res += '\n'
            empty = False
    if empty: res = '查无此人唉-_-'#getAutoResponse(querystr)
    return res
...
def responseMsg(request):  
    ...
    if len([i for i in Student.objects.filter(weixin=openid)]) == 0:
        res = '您的OpenID: ' + openid + '\n小窗等不及了, 快加入我们吧!\n到http://lbscontacts.sinaapp.com注册或加入班级并添加自己的信息~'
    elif 'voice' == msgtype:
        res = '声音啥的小窗真的听不懂呀~'
    elif 'image' == msgtype:
        res = '小窗不近美色不看帅哥~'
