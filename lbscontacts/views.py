...
def enter_class(request, classid):
    ...
    bucket = Bucket('lbscontacts')
    d = [ i['name'] for i in bucket.list() ]
    '''for bae
    HOST = const.BCS_ADDR
    AK = const.ACCESS_KEY
    SK = const.SECRET_KEY
    ...
    if str(classid) == str(request.user):
        ...
        for i in Student.objects.filter(classs=Class(classid=classid)):
            ...
            elif i.birthday[0] == 'g':birth = i.birthday[1:].encode('utf8')+'(公历)'
            else:birth = i.birthday[1:].encode('utf8')+'(农历)'
        ...
        if form.is_valid():
            ...
            answer3 = form.cleaned_data['answer3']
            c = [i for i in Class.objects.filter(classid=classid)][0]
            if answer1 in c.answer1.split('|') and\
                answer2 in c.answer2.split('|') and\
                answer3 in c.answer3.split('|'):
                content.update({'validate':False, 'profile':True})
                for i in Student.objects.filter(classs=Class(classid=classid)):
                    try:
                        pt = re.compile(r'\(\d+\.\d+,\d+\.\d+\)')
